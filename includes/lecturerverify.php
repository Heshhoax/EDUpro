...
if (isset($_POST["submit"])) {
    ...
    if (empty($username) || empty($password)) {
        header("location: ../lecturerlogin.php?error=emptyfields");
        exit();
    }

    // Query database to check if admin credentials are valid
    $query = "SELECT * FROM lecturerinfo WHERE username = '$username' AND password = '$password'";
    $result = mysqli_query($conn, $query);

    if (!$result || mysqli_num_rows($result) == 0) {
        // No admin found with the provided credentials
        header("location: ../lecturerlogin.php?error=invalidcredentials");
        exit();
    } else {
